...
namespace AltAI
{
            ...
            ConstructItem getSelection()
            {
                ...
                else
                {
                    ...
                    if (rankAndMaxProduction.first <= cityCount / 2)
                    {
                        ...
                        if (!possibleSettlers.empty() && existingSettlersCount < 1 + (player.getSettlerManager()->getBestCitySites(140, 4).size() / 2) && setConstructItem(*possibleSettlers.begin()))
                        {
#ifdef ALTAI_DEBUG
                            os << "\n(getConstructItem) Returning settler unit: " << gGlobals.getUnitInfo(*possibleSettlers.begin()).getType() << selection;
#endif
                        ...
                        if (!possibleSettlers.empty() && existingSettlersCount < 1 + (player.getSettlerManager()->getBestCitySites(100, 4).size() / 2) && setConstructItem(*possibleSettlers.begin()))
                        {
#ifdef ALTAI_DEBUG
                            os << "\n(getConstructItem) Returning settler unit: " << gGlobals.getUnitInfo(*possibleSettlers.begin()).getType() << selection;
#endif
    ...
    ConstructItem selectExpansionBuildingTactics(const Player& player, const City& city, const ConstructItem& constructItem)
    {
        ...
        //if (pCity->canConstruct(constructItem.buildingType) && pCity->getFirstBuildingOrder(constructItem.buildingType) == -1)
        {
            ...
            bool needCulture = pCity->getCultureLevel() == 1 && pCity->getCommerceRate(COMMERCE_CULTURE) == 0 && !canBuildCulture;
            ...
            if (selectedConstructItem.isEmpty() && isProcess && (!isBuildCulture || needCulture && isBuildCulture))
            {
