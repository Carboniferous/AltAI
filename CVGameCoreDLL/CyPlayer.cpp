...
// AltAI
bool CyPlayer::isUsingAltAI()
{
    return m_pPlayer ? m_pPlayer->isUsingAltAI() : false;
}

// AltAI
void CyPlayer::setUsingAltAI(bool flag)
{
    if (m_pPlayer)
    {
        m_pPlayer->setUsingAltAI(flag);
    }
}
...
{
	return m_pPlayer && m_pPlayer->getID() != ePlayer ? m_pPlayer->AI_getAttitude((PlayerTypes)ePlayer) : NO_ATTITUDE;
}
