...
void CvCity::kill(bool bUpdatePlotGroups)
{
    ...
    // AltAI
    GC.getGame().getAltAI()->getPlayer(m_eOwner)->deleteCity(this);
...
void CvCity::changePopulation(int iChange)
{
    ...
    {
        // AltAI - set flag to recalc shared plots
        GC.getGame().getAltAI()->getPlayer(m_eOwner)->setCityDirty(getIDInfo());
    }
}
		...
		if (getCultureLevel() != NO_CULTURELEVEL)
		{
			for (iDX = -getCultureLevel(); iDX <= getCultureLevel(); iDX++)
			{
				for (iDY = -getCultureLevel(); iDY <= getCultureLevel(); iDY++)
				{
					...
					if (iCultureRange > eOldValue)  // e.g. 2 > 1 for a newly acquired plot after first border expansion
					{
						if (iCultureRange <= getCultureLevel())  // 2 <= 2 for same example
						{
							FAssert(iCultureRange <= GC.getNumCultureLevelInfos());
							...
							if (pLoopPlot != NULL)
							{
                                // so, plot cultural range data for this player will have 1 added to its range[2] value
                                // indicating that the plot is in the 2nd cultural ring of a city and is under its influence (but not saying how strong its influence is)
								pLoopPlot->changeCultureRangeCities(getOwnerINLINE(), iCultureRange, 1, bUpdatePlotGroups);  
							}
						}
					}
...
void CvCity::read(FDataStreamBase* pStream)
{
    ...
    // AltAI (add the city, even if we are not controlling it)
    GC.getGame().getAltAI()->getPlayer(m_eOwner)->addCity(this);

    if (GET_PLAYER(getOwnerINLINE()).isUsingAltAI())
    {
        GC.getGame().getAltAI()->getPlayer(m_eOwner)->getCity(m_iID).read(pStream);
    }
}
...
void CvCity::write(FDataStreamBase* pStream)
{
    ...
    if (GET_PLAYER(getOwnerINLINE()).isUsingAltAI())
    {
        GC.getGame().getAltAI()->getPlayer(m_eOwner)->getCity(m_iID).write(pStream);
    }
}
...
public:
    ...
    // this does the same thing!!! (and is const-correct)
    //bool operator() (BuildingTypes e1, BuildingTypes e2) const
    //{
    //    const CvBuildingInfo& info1 = GC.getBuildingInfo(e1), &info2 = GC.getBuildingInfo(e2);
    //    const int vis1 = info1.getVisibilityPriority(), vis2 = info2.getVisibilityPriority();
    //    return vis1 == vis2 ? e1 > e2 : vis1 > vis2;
    //}
};
