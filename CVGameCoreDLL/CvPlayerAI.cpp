...
void CvPlayerAI::AI_updateFoundValues(bool bStartingLoc) const
{
	...
	else
	{
        ...
        // AltAI
        if (gGlobals.getGame().getAltAI()->isInit() && m_bUsingAltAI)
        {
            GC.getGame().getAltAI()->getPlayer(m_eID)->updatePlotValues();
        }
		...
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			if (pLoopPlot->isRevealed(getTeam(), false))// || AI_isPrimaryArea(pLoopPlot->area()))
			{
				...
				if (lResult == -1)
				{
                    ...
                    if (m_bUsingAltAI)
                    {
                        iValue = pLoopPlot->getFoundValue(m_eID);  // found values will have been set by MapAnalysis code in AltAI
                    }
                    else
                    {
        ...
        if (gGlobals.getGame().getAltAI()->isInit() && m_bUsingAltAI)
        {
            GC.getGame().getAltAI()->getPlayer(m_eID)->logCitySites();
        }
	}
}
...
TechTypes CvPlayerAI::AI_bestTech(int iMaxPathLength, bool bIgnoreCost, bool bAsync, TechTypes eIgnoreTech, AdvisorTypes eIgnoreAdvisor) const
{
    ...
    if (m_bUsingAltAI)
    {
        GC.getGame().getAltAI()->getPlayer(m_eID)->logBestResearchTech(eBestTech);
    }
...
void CvPlayerAI::AI_chooseResearch()
{
	...
	if (getCurrentResearch() == NO_TECH)
	{
		...
		if (eBestTech != NO_TECH)
		{
            // AltAI - clear current queue when we push a tech
            pushResearch(eBestTech, m_bUsingAltAI);
		}
	}
}
...
void CvPlayerAI::AI_doCommerce()
{
    ...
    if (isUsingAltAI())
    {
        ...
        if (getGold() > 300 && maxResearchRate < 90)
        {
            maxResearchRate += 20;
        }
        else if (getGold() > 100 && maxResearchRate < 100)
        {
            maxResearchRate += 10;
        }
        ...
        if (isCommerceFlexible(COMMERCE_RESEARCH))
        {
            ...
            {
                setCommercePercent(COMMERCE_RESEARCH, maxResearchRate);
            }
            return;
        }
    }
...
void CvPlayerAI::AI_updateCitySites(int iMinFoundValueThreshold, int iMaxSites) const
{
    if (m_bUsingAltAI)
    {
        m_aiAICitySites = GC.getGame().getAltAI()->getPlayer(m_eID)->getBestCitySites(iMinFoundValueThreshold, iMaxSites);
        return;
    }
	...
	while (iPass < iMaxSites)
	{
		...
		if (pBestFoundPlot != NULL)
		{
            ...
            // AltAI - don't want to update again as the algorithm doesn't care about selected sites unless they become cities
            if (!m_bUsingAltAI)
            {
