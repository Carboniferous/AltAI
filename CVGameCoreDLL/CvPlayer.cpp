...
{
    // AltAI
    void addPlayer(CvPlayer* thisPlayer)
    {
        GC.getGame().getAltAI()->addPlayer(thisPlayer);
    }

    void initPlayer(CvPlayer* thisPlayer)
    {
        GC.getGame().getAltAI()->addPlayerDone(thisPlayer);

        // last one - nearly done
        if (thisPlayer->getID() == BARBARIAN_PLAYER)
        {
            ...
            while (const CvPlayerAI* player = playerIter())
            {
                ...
                {
                    GC.getGame().getAltAI()->getPlayer(player->getID())->init();
                }
            }

            GC.getGame().getAltAI()->setInit(true);
        }
    }
}
...
/*************************************************************************************************/
    ...
    // AltAI
    m_bUsingAltAI = false;  // default value
...
void CvPlayer::init(PlayerTypes eID)
{
    ...
    else if (gGlobals.getGame().isOption(GAMEOPTION_ALTAI))
    {
        // See how many we have so far
        AltAI::PlayerIter playerIter;
        int initCount = 0, usingAltAICount = 0;
        while (const CvPlayerAI* player = playerIter())
        {
            boost::shared_ptr<AltAI::Player> pAltAIPlayer = GC.getGame().getAltAI()->getPlayer(player->getID());
            if (pAltAIPlayer)  // means we have called addPlayer()
            {
                ++initCount;
            }
            if (player->isUsingAltAI())
...
// AltAI
bool CvPlayer::isUsingAltAI() const
{
    return m_bUsingAltAI;
}

// AltAI
void CvPlayer::setUsingAltAI(bool flag)
{
    m_bUsingAltAI = flag;
}
...
void CvPlayer::foundReligion(ReligionTypes eReligion, ReligionTypes eSlotReligion, bool bAward)
{
    ...
    while (const CvPlayerAI* player = playerIter())
    {
        if (player->isUsingAltAI())
        {
            GC.getGame().getAltAI()->getPlayer(player->getID())->notifyReligionFounded(eSlotReligion, player->getID() == getID());
        }
    }
...
bool CvPlayer::pushResearch(TechTypes eTech, bool bClear)
{
  ...
  TechTypes eShortestOr;
    ...
    // AltAI
    GC.getGame().getAltAI()->getPlayer(getID())->logPushResearchTech(eTech);
...
void CvPlayer::read(FDataStreamBase* pStream)
{
    ...
    if (m_bUsingAltAI)
    {
        GC.getGame().getAltAI()->getPlayer(m_eID)->read(pStream);
    }
}
...
void CvPlayer::write(FDataStreamBase* pStream)
{
    ...
    if (m_bUsingAltAI)
    {
        GC.getGame().getAltAI()->getPlayer(m_eID)->write(pStream);
    }
}
