...
namespace AltAI
{
    ...
    void CivicTactics::apply(TacticSelectionDataMap& selectionDataMap, int depTacticFlags)
    {
    ...
    void CivicTactics::apply(TacticSelectionData& selectionData)
    {
        ...
        selectionData.civicValues.push_back(civicValue);
    ...
    CivicTacticsPtr CivicTactics::factoryRead(FDataStreamBase* pStream)
    {
        ...
        case CivicTacticsID:
            ...
            break;
        default:
            FAssertMsg(false, "Unexpected ID in CivicTactics::factoryRead");
            break;
        }
    ...
    void EconomicCivicTactic::apply(const CivicTacticsPtr& pCivicTactics, TacticSelectionData& selectionData)
    {
        ...
        for (std::map<IDInfo, ProjectionLadder>::const_iterator ci(cityProjections_.begin()), ciEnd(cityProjections_.end()); ci != ciEnd; ++ci)
        {
            ...
            TotalOutput base = gGlobals.getGame().getAltAI()->getPlayer(ci->first.eOwner)->getCity(ci->first.iID).getBaseOutputProjection().getOutput();
            TotalOutput delta = ci->second.getOutput() - base;
            civicValue.outputDelta += delta;
    ...
    void HurryCivicTactic::update(const CivicTacticsPtr& pCivicTactics, Player& player)
    {
        ...
        while (CvCity* pCity = cityIter())
        {
            ...
            // if we can't hurry population skip this city
            // could improve this by making the hurry event below conditional on pop reaching 2
            if (pCity->getPopulation() < 2 && productionPerPopulation_ > 0)
            {
                cityProjections_[pCity->getIDInfo()] = std::make_pair(base, base);  // no effect
                continue;
            }            
            ...
            hurryData.hurryPopulation = std::min<int>(2, pCityData->getPopulation()); //  todo - derive dynamically
    ...
    {
        pStream->Write(ID);
        pStream->Write(productionPerPopulation_);
        pStream->Write(goldPerProduction_);
        // todo - save projection data
    }

    void HurryCivicTactic::read(FDataStreamBase* pStream)
    {
        pStream->Read(&productionPerPopulation_);
        pStream->Read(&goldPerProduction_);
    }

    HappyPoliceCivicTactic::HappyPoliceCivicTactic(int happyPerUnit) : happyPerUnit_(happyPerUnit)
    {
    }

    void HappyPoliceCivicTactic::debug(std::ostream& os) const
    {
#ifdef ALTAI_DEBUG
        os << "\n\tHappy police tactic: " << happyPerUnit_ << " happy pop per unit in city ";
#endif
    }

    void HappyPoliceCivicTactic::update(const CivicTacticsPtr& pCivicTactics, Player& player)
    {
        CivicOptionTypes civicOptionType = (CivicOptionTypes)gGlobals.getCivicInfo(pCivicTactics->getCivicType()).getCivicOptionType();
        CityIter cityIter(*player.getCvPlayer());
        while (CvCity* pCity = cityIter())
        {
            ...
            if (pCityData->getHappyCap() <= 0)
            {
                pCityData->getHappyHelper()->changeMilitaryHappinessUnits(1);
            }
    ...
    void HappyPoliceCivicTactic::apply(const CivicTacticsPtr& pCivicTactics, TacticSelectionData& selectionData)
    {
        CivicValue& civicValue = *selectionData.civicValues.rbegin();
        civicValue.yieldOutputDelta = std::make_pair(TotalOutput(), TotalOutput());
        ...
        for (std::map<IDInfo, std::pair<ProjectionLadder, ProjectionLadder> >::const_iterator ci(cityProjections_.begin()), ciEnd(cityProjections_.end()); ci != ciEnd; ++ci)
        {
            ...
            accumulatedBase += ci->second.second.getOutput();
            accumulatedDelta += ci->second.first.getOutput() - ci->second.second.getOutput();
            // todo - subtract production cost (and maintenance?) of any additional military happiness units
        }
        civicValue.yieldOutputDelta.first += accumulatedDelta;
        civicValue.yieldOutputDelta.second += asPercentageOf(accumulatedDelta, accumulatedBase);
    }

    void HappyPoliceCivicTactic::write(FDataStreamBase* pStream) const
    {
        pStream->Write(ID);
        pStream->Write(happyPerUnit_);
    }

    void HappyPoliceCivicTactic::read(FDataStreamBase* pStream)
    {
        pStream->Read(&happyPerUnit_);
    }
}