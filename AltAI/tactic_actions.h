...
namespace AltAI
{
    ...
    struct TechFlags
    {
        enum Flags
        {
            None = 0, Found_Religion = (1 << 0), Free_Tech = (1 << 2), Free_GP = (1 << 3), Flexible_Commerce = (1 << 4), Trade_Routes = (1 << 5), Spaceship_Tech = (1 << 6)
        };
    ...
    struct VictoryFlags
    {
        enum Flags
        {
            None = 0, Prereq_Project = (1 << 0), Component_Project = (1 << 2)
        };

        VictoryFlags() : flags(None) {}
    ...
    struct ResearchTech
    {
        ...
        explicit ResearchTech(TechTypes techType_, int depth_ = 1)
            : techType(techType_), targetTechType(techType_), depth(depth_),
              techFlags(0), economicFlags(0), militaryFlags(0), workerFlags(0), victoryFlags(0)
        {
        }

        TechTypes techType, targetTechType;
        int depth;
        int techFlags, economicFlags, militaryFlags, workerFlags, victoryFlags;
        std::vector<BuildingTypes> possibleBuildings;
        std::vector<ProjectTypes> possibleProjects;
        ...
        bool hasFlags() const
        {
            return techFlags || economicFlags || militaryFlags || workerFlags || victoryFlags;
        }

        bool isEmpty() const
        {
            return !hasFlags() && possibleBuildings.empty() && possibleProjects.empty() && possibleBonuses.empty() && possibleCivics.empty() && 
    ...
    struct ConstructItem
    {
        ConstructItem()
            : buildingType(NO_BUILDING), unitType(NO_UNIT), projectType(NO_PROJECT), improvementType(NO_IMPROVEMENT), processType(NO_PROCESS),
              economicFlags(0), militaryFlags(0), buildingFlags(0), victoryFlags(0) {}
        ...
        bool hasFlags() const
        {
            return economicFlags || militaryFlags || buildingFlags || victoryFlags;
        }
        ...
        int economicFlags, militaryFlags, buildingFlags, victoryFlags;
        std::vector<ConstructItem> prerequisites;
    ...
    struct ConstructItemFinder
    {
        ...
        bool operator() (const ConstructItem& other) const
        {
            return constructItem.buildingType == other.buildingType && constructItem.unitType == other.unitType &&
                ...
                constructItem.projectType == other.projectType;
